...
2. 时间同步。
    ...
    # 写入 crontab 计划任务
    cat << EOF >> /etc/crontab
    0da */1 * * * ntpdate time1.aliyun.com
    EOF
    # 使用 date 命令验证时间
    date
    ```
...
# 这里安装的是精简版，后续如果要构建镜像，可能还需要安装其他组件，并且可能无法使用配置好的镜像加速，建议安装full版本
wget https://github.com/containerd/nerdctl/releases/download/v2.1.1/nerdctl-full-2.1.1-linux-amd64.tar.gz

# 解压，只需要把其中的 nerdctl 可执行文件放到 /usr/local/bin 就行了
tar -zxvf nerdctl-2.0.0-linux-amd64.tar.gz nerdctl
mv nerdctl /usr/local/bin
# full 版本安装如下
tar -zxf nerdctl-full-2.1.1-linux-amd64.tar.gz -C /usr/local
...
* 提前在各个节点上拉取Kubernetes需要的镜像
  ...
  # 或者使用nerdctl
  nerdctl -n k8s.io pull registry.k8s.io/kube-apiserver:v1.28.15
  nerdctl -n k8s.io pull registry.k8s.io/kube-controller-manager:v1.28.15
  nerdctl -n k8s.io pull registry.k8s.io/kube-scheduler:v1.28.15
  nerdctl -n k8s.io pull registry.k8s.io/kube-proxy:v1.28.15
  nerdctl -n k8s.io pull registry.k8s.io/pause:3.9
  nerdctl -n k8s.io pull registry.k8s.io/etcd:3.5.15-0
  nerdctl -n k8s.io pull registry.k8s.io/coredns/coredns:v1.10.1
  ~~~
...
etcd:
  local:
    ...
    extraArgs:
      listen-client-urls: https://0.0.0.0:2379 #直接在配置文件指定，用于在ECS服务器上部署Kubernetes，非ECS服务器可以不配
      listen-peer-urls: https://0.0.0.0:2380 #直接在配置文件指定，用于在ECS服务器上部署Kubernetes，非ECS服务器可以不配
