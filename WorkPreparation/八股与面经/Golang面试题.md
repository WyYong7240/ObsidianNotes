[\# Golang面试题](https://xiaolincoding.com/interview/golang.html#_12-8-%E4%BD%BF%E7%94%A8%E4%B8%A4%E4%B8%AAgoroutine-%E5%90%91%E6%A0%87%E5%87%86%E8%BE%93%E5%87%BA%E4%B8%AD%E6%8C%89%E9%A1%BA%E5%BA%8F%E6%8C%89%E9%A1%BA%E5%BA%8F%E4%BA%A4%E6%9B%BF%E6%89%93%E5%87%BA%E5%AD%97%E6%AF%8D%E4%B8%8E%E6%95%B0%E5%AD%97-%E8%BE%93%E5%87%BA%E6%98%AFa1b2c3)
...
func main() {
  ...
  fmt.Printf("a=%v  n", a) // a=&{1 微客鸟窝}  
  fmt.Printf("a=%+v  n", a) // a=&{id:1 name:微客鸟窝}  
  fmt.Printf("a=%#v  n", a) // a=&main.student{id:1, name:"微客鸟窝"}
}
...
func printGCStats() {
        ...
        for {
                ...
                case <-t.C:
                        debug.ReadGCStats(&s)
                        fmt.Printf("gc %d last@%v, PauseTotal %vn", s.NumGC, s.LastGC, s.PauseTotal)
                }
        }
}
func main() {
...
func printMemStats() {
        ...
        for {
                ...
                case <-t.C:
                        ...
                        fmt.Printf("gc %d last@%v, next_heap_size@%vMBn", s.NumGC, time.Unix(int64(time.Duration(s.LastGC).Seconds()), 0), s.NextGC/(1<<20))
                }
        }
}
func main() {
    ...
    for i := 0; i < 10; i++ {
       ...
       go func(idx int) {
          ...
          for val := range hashMap[idx] {
             fmt.Printf("go id: %d, val: %d n", idx, val)
             sort <- struct{}{}
          }
       }(i)
    }
    ...
    producer := func(ctx context.Context, out chan<- int, idx int) {
       ...
       for {
          ...
          default:
             ...
             num := rand.Intn(500)
             out <- num
             fmt.Printf("producer: %d, msg: %dn", idx, num)
             cond.Signal()
             cond.L.Unlock()
          }
       }
       ...
       for {
          ...
          case <-ctx.Done():
             ...
             for len(msgCh) > 0 {
                select {
                case num := <-in:
                   fmt.Printf("consumer %d, msg: %dn", idx, num)
                default:
                   // 如果channel已经空了，跳出循环
                   break
                }
